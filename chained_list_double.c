...
void insertList(LL* pList, int idx, int val) {  ////insert 插入 idx 是插入的位置
  ...
  if (pList->pHead == NULL) {
    ...
    pTemp->pFront = pInsertNode;
    if (pInsertNode->pNext) {
      pInsertNode->pNext->pFront = pTemp;
    }
